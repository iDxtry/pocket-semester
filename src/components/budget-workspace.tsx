  ...
  return (
    <div className="app-shell">
      ...
      <main className="dashboard" id="workspace-main" tabIndex={-1}>
        ...
        {activeView === "dashboard" && <><div className="mobile-action-dock" aria-label="Quick actions"><button className="secondary-button" onClick={() => setShowImport(true)}><UploadSimple /> Import CSV</button><button className="primary-button" onClick={() => openExpense()}><Plus weight="bold" /> Add expense</button></div><DashboardView summary={summary} forecast={forecast} fixedSpendCents={fixedSpendCents} forecastDifference={forecastDifference} goal={goalSummary} profile={profile} transactions={monthTransactions} monthState={monthState} monthSeries={monthSeries} streaks={streaks} runway={runway} runwayFundsCents={runwayFundsCents} runwayPurchaseCents={runwayPurchaseCents} runwayActions={runwayActions} coachPlan={visibleCoachPlan} coachProvenance={coachProvenance} coachState={coachState} coachError={coachError} isDemo={mode === "demo"} hasCategorization={Boolean(latestCategorization)} hasUsedRunway={hasUsedRunway} onAddExpense={() => openExpense()} onRefreshCoach={refreshCoach} onFundsChange={(cents) => { setHasUsedRunway(true); setRunwayFundsCents(cents); }} onPurchaseChange={(cents) => { setHasUsedRunway(true); setRunwayPurchaseCents(cents); }} onActionsChange={(ids) => { setHasUsedRunway(true); setRunwayActions(ids); }} onOpenTransactions={() => mode === "demo" ? activateDemoView("transactions") : window.location.assign(routeHref(mode, "transactions", month))} onDrilldown={drillIntoCategory} /></>}
...
function DashboardView({ summary, forecast, fixedSpendCents, forecastDifference, goal, profile, transactions, monthState, monthSeries, streaks, runway, runwayFundsCents, runwayPurchaseCents, runwayActions, coachPlan, coachProvenance, coachState, coachError, isDemo, hasCategorization, hasUsedRunway, onAddExpense, onRefreshCoach, onFundsChange, onPurchaseChange, onActionsChange, onOpenTransactions, onDrilldown }: {
...
function RunwayPanel({ state, runway, currency, availableFundsCents, plannedPurchaseCents, selectedActionIds, onFundsChange, onPurchaseChange, onActionsChange }: { state: ReturnType<typeof getMonthState>; runway: ReturnType<typeof calculateSemesterRunway>; currency: string; availableFundsCents: number; plannedPurchaseCents: number; selectedActionIds: string[]; onFundsChange: (cents: number) => void; onPurchaseChange: (cents: number) => void; onActionsChange: (ids: string[]) => void }) {
  ...
  const displayDate = runway.coveredThrough ? new Intl.DateTimeFormat("en-US", { month: "short", day: "numeric", timeZone: "UTC" }).format(new Date(`${runway.coveredThrough}T12:00:00Z`)) : "—";
