...
import { calculateSemesterRunway } from "@/lib/runway";
...
type ExpenseAnalysisResponse = { category: Category; confidence: number; source: AiSource; model: string | null; rationale: string; insight: string; action: string };
type CoachResponse = { plan: CoachPlan; source: "openai" | "gemini"; model: string };
...
function demoCoachPlanForMonth(month: string, state: ReturnType<typeof getMonthState>): CoachPlan {
  if (state === "past") {
    return { summary: `${monthLabel(month)} is closed. Use this recap to notice the category that moved most, then carry one small adjustment into the next month.`, watchouts: [{ category: "Food & dining", message: "Food stayed the most flexible category in this fictional month." }], actions: [{ title: "Review the biggest flexible category", detail: "Use the final category total to set one realistic change for the next month.", estimatedImpactCents: 1500 }, { title: "Keep the emergency cushion visible", detail: "Move a small amount after the month closes so the goal stays intentional.", estimatedImpactCents: 1000 }], estimatedImpactCents: 2500 };
  }
  if (state === "future") {
    return { summary: `${monthLabel(month)} has not started. This is a planning preview, not an AI-generated forecast.`, watchouts: [{ category: "School", message: "Leave room for course materials and other start-of-month costs." }], actions: [{ title: "Add known fixed costs first", detail: "Start with housing, transit, and subscriptions before deciding on flexible spending.", estimatedImpactCents: 0 }, { title: "Set a weekly food cap", detail: "A small weekly limit makes the first two weeks easier to manage.", estimatedImpactCents: 0 }], estimatedImpactCents: 0 };
  }
  return demoExampleCoachPlan;
}
...
export function BudgetWorkspace({
  ...
  initialCoachProvenance?: CoachProvenance | null;
  ...
  const [coachProvenance, setCoachProvenance] = useState<CoachProvenance | null>(() => initialCoachProvenance ?? (initialCoachPlan ? null : mode === "demo" ? { source: "example", model: null } : null));
  ...
  const [runwayFundsCents, setRunwayFundsCents] = useState(mode === "demo" ? 120_000 : Math.max(initialData.profile.monthlyAllowanceCents, 0));
  const [runwayPurchaseCents, setRunwayPurchaseCents] = useState(mode === "demo" ? 26_000 : 0);
  const [runwayActions, setRunwayActions] = useState<string[]>([]);
  const [hasUsedRunway, setHasUsedRunway] = useState(false);
  ...
  const baselineRunway = useMemo(() => calculateSemesterRunway({ month, profile, transactions: monthTransactions, budgets, availableFundsCents: runwayFundsCents, plannedPurchaseCents: runwayPurchaseCents, selectedActionIds: [] }), [month, profile, monthTransactions, budgets, runwayFundsCents, runwayPurchaseCents]);
  ...
  async function refreshCoach() {
    if (monthState !== "current") {
      setCoachError(monthState === "past" ? "Completed months use a recap instead of a live coach refresh." : "The coach can refresh once this month begins.");
      return;
    }
  ...
  const visibleCoachPlan = mode === "demo" && coachProvenance?.source === "example" ? demoCoachPlanForMonth(month, monthState) : coachPlan;
  return (
    <div className="app-shell">
        ...
        <div className={`account-footer ${mode === "demo" ? "demo-account-footer" : ""}`}>
          {mode === "demo" ? (
            <>
              ...
              <div className="demo-account-actions"><button className="sidebar-signin" onClick={() => window.location.assign("/demo")}>Reset Alex’s demo</button><Link className="sidebar-signout" href="/"><ArrowRight /> Exit demo</Link><Link className="sidebar-signin" href="/sign-in">Sign in</Link></div>
            </>
          ) : (
            <>
      ...
      <main className="dashboard" id="workspace-main" tabIndex={-1}>
        ...
        {activeView === "dashboard" && <><div className="mobile-action-dock" aria-label="Quick actions"><button className="secondary-button" onClick={() => setShowImport(true)}><UploadSimple /> Import CSV</button><button className="primary-button" onClick={() => openExpense()}><Plus weight="bold" /> Add expense</button></div><DashboardView summary={summary} forecast={forecast} fixedSpendCents={fixedSpendCents} forecastDifference={forecastDifference} goal={goalSummary} profile={profile} transactions={monthTransactions} monthState={monthState} monthSeries={monthSeries} streaks={streaks} runway={runway} baselineRunway={baselineRunway} runwayFundsCents={runwayFundsCents} runwayPurchaseCents={runwayPurchaseCents} runwayActions={runwayActions} coachPlan={visibleCoachPlan} coachProvenance={coachProvenance} coachState={coachState} coachError={coachError} isDemo={mode === "demo"} hasCategorization={Boolean(latestCategorization)} hasUsedRunway={hasUsedRunway} onAddExpense={() => openExpense()} onRefreshCoach={refreshCoach} onFundsChange={(cents) => { setHasUsedRunway(true); setRunwayFundsCents(cents); }} onPurchaseChange={(cents) => { setHasUsedRunway(true); setRunwayPurchaseCents(cents); }} onActionsChange={(ids) => { setHasUsedRunway(true); setRunwayActions(ids); }} onOpenTransactions={() => mode === "demo" ? activateDemoView("transactions") : window.location.assign(routeHref(mode, "transactions", month))} onDrilldown={drillIntoCategory} /></>}
...
function DashboardView({ summary, forecast, fixedSpendCents, forecastDifference, goal, profile, transactions, monthState, monthSeries, streaks, runway, baselineRunway, runwayFundsCents, runwayPurchaseCents, runwayActions, coachPlan, coachProvenance, coachState, coachError, isDemo, hasCategorization, hasUsedRunway, onAddExpense, onRefreshCoach, onFundsChange, onPurchaseChange, onActionsChange, onOpenTransactions, onDrilldown }: {
  summary: ReturnType<typeof getBudgetSummary>; forecast: ReturnType<typeof getForecast>; fixedSpendCents: number; forecastDifference: number; goal: ReturnType<typeof goalProgress>; profile: StudentProfile; transactions: BudgetTransaction[]; monthState: ReturnType<typeof getMonthState>; monthSeries: ReturnType<typeof makeMonthSeries>; streaks: ReturnType<typeof getSpendingStreaks>; runway: ReturnType<typeof calculateSemesterRunway>; baselineRunway: ReturnType<typeof calculateSemesterRunway>; runwayFundsCents: number; runwayPurchaseCents: number; runwayActions: string[]; coachPlan: CoachPlan | null; coachProvenance: CoachProvenance | null; coachState: "idle" | "loading"; coachError: string; isDemo: boolean; hasCategorization: boolean; hasUsedRunway: boolean; onAddExpense: () => void; onRefreshCoach: () => void; onFundsChange: (cents: number) => void; onPurchaseChange: (cents: number) => void; onActionsChange: (ids: string[]) => void; onOpenTransactions: () => void; onDrilldown: (category: Category) => void;
}) {
  ...
  return <>
    <section className="metrics" aria-label="Budget summary">
      ...
      <article className="metric"><div className="metric-heading"><span>{forecastTitle}</span><CurrencyDollar /></div><strong>{monthState === "future" ? formatMoney(summary.totalBudgetCents, profile.currency) : formatMoney(forecast.forecastCents, profile.currency)}</strong><p className={forecastDifference >= 0 ? "positive" : "negative"}>{monthState === "past" ? "Month closed" : monthState === "future" ? "No expenses recorded yet" : forecastDifference >= 0 ? <><ArrowDownRight weight="bold" /> {formatMoney(forecastDifference, profile.currency)} below your plan</> : <><ArrowUpRight weight="bold" /> {formatMoney(Math.abs(forecastDifference), profile.currency)} above your plan</>}</p></article>
    ...
    {isDemo && <DemoChecklist hasCategorization={hasCategorization} hasRefreshedPlan={coachProvenance?.source !== "example"} hasRunway={hasUsedRunway} onAddExpense={onAddExpense} onRefreshPlan={onRefreshCoach} />}
    <ForecastExplainer forecast={forecast} fixedSpendCents={fixedSpendCents} currency={profile.currency} state={monthState} />
    <RunwayPanel state={monthState} runway={runway} baselineRunway={baselineRunway} currency={profile.currency} availableFundsCents={runwayFundsCents} plannedPurchaseCents={runwayPurchaseCents} selectedActionIds={runwayActions} onFundsChange={onFundsChange} onPurchaseChange={onPurchaseChange} onActionsChange={onActionsChange} />
    ...
    <section className="lower-grid">
      ...
      <article className="panel budgets-panel"><div className="panel-heading compact"><div><h2>Budget health</h2><p>Tap a category to see every related expense.</p></div></div><div className="budget-chart-container"><BudgetHealthChart summary={summary} currency={profile.currency} /></div><div className="budget-list">{summary.categoryHealth.filter((item) => item.limitCents > 0).slice(0, 5).map((item) => {
        const status = item.remainingCents === 0 ? "At limit" : item.status === "over" ? "Over plan" : item.status === "watch" ? "Watch" : "On track";
        const statusClass = item.remainingCents === 0 ? "at-limit" : item.status;
        return <button className="budget-row budget-row-button" key={item.category} onClick={() => onDrilldown(item.category)}><div><strong>{item.category}</strong><span>{formatMoney(item.spentCents, profile.currency)} of {formatMoney(item.limitCents, profile.currency)}</span></div><div className="budget-row-meta"><small className={`budget-status ${statusClass}`}>{status}</small></div><div className="category-track"><span style={{ width: `${Math.min(item.percentUsed, 100)}%`, background: categoryColors[item.category] }} /></div></button>;
      })}</div></article>
    </section>
  </>;
}
...
function BudgetHealthChart({ summary, currency }: { summary: ReturnType<typeof getBudgetSummary>; currency: string }) {
  ...
  if (data.length === 0) {
    return <div className="budget-chart-empty" aria-hidden="true"><ChartDonut /><span>No spending yet</span></div>;
  }

  return (
    <div className="budget-health-chart" aria-hidden="true">
      ...
      <div className="budget-chart-center-label">
        <strong>{formatMoney(summary.totalSpentCents, currency)}</strong>
        <small>Spent</small>
      </div>
    </div>
  );
...
function WeeklyDigestCard({ digest, currency, coachPlan, provenance, runway }: { digest: ReturnType<typeof getWeeklySpendingDigest>; currency: string; coachPlan: CoachPlan | null; provenance: CoachProvenance | null; runway: ReturnType<typeof calculateSemesterRunway> }) {
  ...
  return (
    <article className="coach-card weekly-digest-card">
      ...
      {generatedNote ? <div className="weekly-coach-note"><small>Plan action</small><strong>{generatedNote.title}</strong><span>{generatedNote.detail}</span></div> : <p className="weekly-digest-prompt">Refresh your spending plan to add a personalized plan action.</p>}
    </article>
  );
}
...
function DemoChecklist({ hasCategorization, hasRefreshedPlan, hasRunway, onAddExpense, onRefreshPlan }: { hasCategorization: boolean; hasRefreshedPlan: boolean; hasRunway: boolean; onAddExpense: () => void; onRefreshPlan: () => void }) {
  const allComplete = hasCategorization && hasRunway && hasRefreshedPlan;
  const narrative = (
    <div className="demo-progress-narrative">
  ...
  if (allComplete) {
    return <section className="demo-progress" aria-label="Guided tour complete"><div className="demo-progress-heading"><span>Tour complete 🎉</span><p>You&apos;ve seen the core features.</p></div><div className="demo-progress-body">{narrative}<Link href="/sign-up" className="primary-button demo-progress-account-link">Create a real account</Link></div></section>;
  }
  return <section className="demo-progress" aria-label="5-step guided tour"><div className="demo-progress-heading"><span>Guided tour</span><p>Fictional data · resets on reload</p></div><div className="demo-progress-body">{narrative}<ol><li className="complete"><span><CheckCircle weight="fill" /></span><strong>See your month at a glance</strong></li><li className={hasCategorization ? "complete" : ""}><span>{hasCategorization ? <CheckCircle weight="fill" /> : "2"}</span><strong>Add an expense</strong><button className="text-button" onClick={onAddExpense}>Try it <ArrowRight /></button></li><li className={hasCategorization ? "complete" : ""}><span>{hasCategorization ? <CheckCircle weight="fill" /> : "3"}</span><strong>Watch AI categorize it</strong></li><li className={hasRunway ? "complete" : ""}><span>{hasRunway ? <CheckCircle weight="fill" /> : "4"}</span><strong>Check the scenario planner</strong></li><li className={hasRefreshedPlan ? "complete" : ""}><span>{hasRefreshedPlan ? <CheckCircle weight="fill" /> : "5"}</span><strong>Get a personalized plan</strong><button className="text-button" onClick={onRefreshPlan}>Refresh <ArrowRight /></button></li></ol></div></section>;
}
...
function ForecastExplainer({ forecast, fixedSpendCents, currency, state }: { forecast: ReturnType<typeof getForecast>; fixedSpendCents: number; currency: string; state: ReturnType<typeof getMonthState> }) {
  const copy = state === "past" ? "This month is complete, so Pocket Semester shows final spending instead of predicting the rest of it." : state === "future" ? "This month has not started. The plan is visible, but there is no real spending pace or live AI forecast yet." : "Housing and subscriptions are fixed. Everything else follows the flexible pace you have set so far.";
  return <section className="forecast-explainer" aria-label="How the month-end forecast works"><div><p className="eyebrow">Forecast</p><h2>{state === "past" ? "What this month actually cost." : state === "future" ? "What this plan sets aside." : "Where this month is headed."}</h2><p>{copy}</p></div><dl><div><dt>Fixed spending</dt><dd>{formatMoney(fixedSpendCents, currency)}</dd></div><div><dt>Flexible daily pace</dt><dd>{state === "current" ? formatMoney(forecast.flexibleDailyPaceCents, currency) : "—"}</dd></div><div><dt>{state === "past" ? "Final month total" : "Projected month end"}</dt><dd>{formatMoney(forecast.forecastCents, currency)}</dd></div><div><dt>{state === "current" ? "Days left" : "Month status"}</dt><dd>{state === "current" ? forecast.daysRemaining : state === "past" ? "Closed" : "Not started"}</dd></div></dl></section>;
}

const purchasePresets = [
  { id: "flight", label: "Flight home", cents: 26_000 },
  { id: "textbooks", label: "Textbooks", cents: 22_000 },
  { id: "concert", label: "Concert ticket", cents: 18_000 },
] as const;

function RunwayPanel({ state, runway, baselineRunway, currency, availableFundsCents, plannedPurchaseCents, selectedActionIds, onFundsChange, onPurchaseChange, onActionsChange }: { state: ReturnType<typeof getMonthState>; runway: ReturnType<typeof calculateSemesterRunway>; baselineRunway: ReturnType<typeof calculateSemesterRunway>; currency: string; availableFundsCents: number; plannedPurchaseCents: number; selectedActionIds: string[]; onFundsChange: (cents: number) => void; onPurchaseChange: (cents: number) => void; onActionsChange: (ids: string[]) => void }) {
  const [fundsInput, setFundsInput] = useState(String(availableFundsCents / 100));
  const [purchaseInput, setPurchaseInput] = useState(String(plannedPurchaseCents / 100));

  if (state !== "current") return <section className="runway-panel runway-unavailable"><div><p className="eyebrow">Semester runway</p><h2>{state === "past" ? "Runway checks are for active months." : "Your runway starts when this month does."}</h2><p>{state === "past" ? "Choose the current month to test a purchase against the rest of your term." : "Set your expected income and expenses once the month begins."}</p></div></section>;

  const covered = runway.status === "covered";
  const displayScenarioDate = runway.coveredThrough ? displayDate(runway.coveredThrough) : "—";
  const displayBaselineDate = baselineRunway.coveredThrough ? displayDate(baselineRunway.coveredThrough) : "—";
  const hasTradeoff = selectedActionIds.length > 0;
  const resourceLabel = runway.selectedImpactCents > 0 ? "Resources after choices" : "Available funds";

  function toggleAction(id: string) {
    onActionsChange(selectedActionIds.includes(id) ? selectedActionIds.filter((item) => item !== id) : [...selectedActionIds, id]);
  }

  function commitMoney(value: string, fallbackCents: number, onCommit: (cents: number) => void, setValue: (next: string) => void) {
    const normalized = value.replace(/,/g, "").trim();
    const amount = Number(normalized);
    if (!normalized || !Number.isFinite(amount) || amount < 0) {
      setValue(String(fallbackCents / 100));
      return;
    }
  ...
  function coverageCopy(value: ReturnType<typeof calculateSemesterRunway>, date: string) {
    return value.status === "covered"
      ? `covered through finals, ${formatMoney(value.finalBufferCents, currency)} buffer`
      : `runs out after ${date}, ${formatMoney(value.shortfallCents, currency)} short`;
  }

  return <section className="runway-panel" aria-label="Semester runway scenario">
    <div className="runway-heading"><div><p className="eyebrow">Semester runway</p><h2>{covered ? `Covered through finals · ${displayScenarioDate}` : `Funds run out after ${displayScenarioDate}`}</h2><p>Calculated from your current pace, category plan, planned purchase, and days through finals. This is a planning estimate, not financial advice.</p></div><div className={`runway-result ${covered ? "covered" : "shortfall"}`}><span>{covered ? "Finals buffer" : "Projected shortfall"}</span><strong>{formatMoney(covered ? runway.finalBufferCents : runway.shortfallCents, currency)}</strong></div></div>
    {hasTradeoff && <p className="runway-comparison"><strong>Before:</strong> {coverageCopy(baselineRunway, displayBaselineDate)} <ArrowRight weight="bold" aria-hidden="true" /> <strong>After:</strong> {coverageCopy(runway, displayScenarioDate)}</p>}
    <div className="runway-timeline"><div className="runway-timeline-track"><div className="runway-timeline-fill" style={{ width: `${Math.min((runway.effectiveResourcesCents / (runway.projectedNeedCents || 1)) * 100, 100)}%`, background: covered ? "var(--accent)" : "var(--danger)" }} /><div className="runway-timeline-marker" style={{ left: "100%" }} /></div><div className="runway-timeline-labels"><span>{resourceLabel}: {formatMoney(runway.effectiveResourcesCents, currency)}</span><span>Need: {formatMoney(runway.projectedNeedCents, currency)}</span></div></div>
    <div className="runway-presets" aria-label="Planned purchase presets"><span>Try a purchase</span>{purchasePresets.map((preset) => <button type="button" key={preset.id} className={plannedPurchaseCents === preset.cents ? "selected" : ""} aria-pressed={plannedPurchaseCents === preset.cents} onClick={() => { setPurchaseInput(String(preset.cents / 100)); onPurchaseChange(preset.cents); }}>{preset.label} · {formatMoney(preset.cents, currency)}</button>)}</div>
    <div className="runway-grid"><label><span>Funds available through finals</span><span className="currency-field"><b>$</b><input aria-label="Funds available through finals" type="text" inputMode="decimal" value={fundsInput} onChange={(event) => setFundsInput(event.target.value)} onBlur={() => commitMoney(fundsInput, availableFundsCents, onFundsChange, setFundsInput)} onKeyDown={commitOnEnter} /></span></label><label><span>Planned purchase</span><span className="currency-field"><b>$</b><input aria-label="Planned purchase" type="text" inputMode="decimal" value={purchaseInput} onChange={(event) => setPurchaseInput(event.target.value)} onBlur={() => commitMoney(purchaseInput, plannedPurchaseCents, onPurchaseChange, setPurchaseInput)} onKeyDown={commitOnEnter} /></span></label><span><small>Need through finals</small><strong>{formatMoney(runway.projectedNeedCents, currency)}</strong></span><span><small>Days to finals</small><strong>{runway.daysToFinals}</strong></span></div>
    <div className="runway-actions"><p><strong>Try a tradeoff</strong><span>Each impact is calculated before the coach explains it.</span></p>{runway.actions.map((action) => <label key={action.id} className={selectedActionIds.includes(action.id) ? "selected" : ""}><input type="checkbox" checked={selectedActionIds.includes(action.id)} onChange={() => toggleAction(action.id)} /><span><strong>{action.title}</strong><small>{action.detail}</small></span><b>+{formatMoney(action.impactCents, currency)}</b></label>)}</div>
  </section>;
}
...
function LatestCategorizationCard({ value, currency, runway, runwayAvailable, onDismiss, onChangeCategory }: { value: LatestCategorization; currency: string; runway: ReturnType<typeof calculateSemesterRunway>; runwayAvailable: boolean; onDismiss: () => void; onChangeCategory: () => void }) {
  ...
  const provenanceDetail = analysis.source === "gemini" ? "Live response · Gemini" : analysis.source === "openai" ? "Live response · OpenAI" : null;
